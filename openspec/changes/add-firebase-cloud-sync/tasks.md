...
- [x] 7.4 Forgot-password dialog wired in [AccountSignInScreen.kt](app/src/main/java/eu/kanade/presentation/more/settings/screen/cloudsync/AccountSignInScreen.kt). Single text field for username-or-email, generic "If an email is on file, a reset link has been sent" toast on submit regardless of whether the account exists. Reachable via "Forgot password?" link on the Sign In tab.
- [x] 7.5 Sign-out is exposed directly in Settings → Cloud sync via [SettingsDataScreen.kt:140](app/src/main/java/eu/kanade/presentation/more/settings/screen/SettingsDataScreen.kt) `SignOutConfirmDialog`. Sync now button also lives in Settings. A dedicated AccountScreen would be redundant for v1; "Delete account" can be added as a follow-up task once the account-management UX is stable.
- [x] 7.6 `SignUpExistingLibraryModal` rendered after a successful sign-up when `decideOnSignUp` returns `AskUser`. Two actions: "Upload existing library" (calls `engine.pushSnapshotIfDirty(force = true)`), "Start fresh" (also pushes once so the cloud has a record). Both pop the navigator after the engine call.
- [x] 7.7 `SignInConflictModal` rendered after a successful sign-in when `decideOnSignIn` returns `AskUser`. Three actions: "Use cloud" (pull-and-restore), "Use local" (force push), "Cancel sign-in" (signs out and stays). Modal cannot be dismissed without an explicit choice.
